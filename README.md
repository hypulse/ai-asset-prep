...
이미지 생성 기능은 `OPENAI_API_KEY` 환경변수 또는 프로젝트 루트의 `.env` 파일이 필요합니다.
...
```bash
python cli.py generate --prompt "pixel art skeleton warrior, transparent background" --background transparent --output outputs/skeleton.png
...
- 기본 이미지 생성 설정: `gpt-image-1-mini`, `1024x1024`, `low`, `png`
...
- `이미지 생성`: OpenAI 이미지 생성 API로 에셋을 만들고, 생성 결과를 다운로드/저장하거나 기존 편집 탭의 소스로 넘깁니다.
